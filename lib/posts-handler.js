...
function handle(req, res) {
    ...
    case 'POST':
        ...
        req.on('data', (chunk) => {
            ...
            const value = Number(decodeURIComponent(Buffer.concat(body).toString()).split('weight=')[1]);
            // data.push({user: req.user, date: getDateString(new Date()), weight: weight});
            console.info('登録されました: ', value);
